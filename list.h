...
void lstConstructor (list* p_lst, size_t list_size)
{
    ...
    NEXT = (int*) calloc (p_lst->max_size, sizeof (elem_t));
    PREV = (int*) calloc (p_lst->max_size, sizeof (elem_t));
...
int InsertHead (list* p_lst, elem_t val);
int InsertTail (list* p_lst, elem_t val);
...
/*int find (list* p_lst, int val)
{
    ...
    for (int i = p_lst->head; i != p_lst->tail; i = NEXT[i])
    {
        if (DATA[i] == val)
            return i;
    }
...
//рисовать в физической последовательности
...
void Unittest1 ()
{
    ...
    system ("\"D:\\Program Files\\Graphviz\\bin\\dot\" -Tpng D:\\Code\\List\\list.txt -o D:\\Code\\List\\image.png");
    system ("start /max D:\\Code\\List\\image.png");
...
void Unittest2 ()
{
    ...
    int phys = InsertAfter (&lst, 1, 11);

    int phys2 = InsertAfter (&lst, phys, 12);
